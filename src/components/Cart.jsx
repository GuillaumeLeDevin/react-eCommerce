            ...
            <ul>
                ...
                cart.cartItems && cart.cartItems.map(item => (
                    <li className="flex items-center mb-4" key={item.id}>
                        ...
                        className="bg-slate-900 text-slate-200 inline-flex items-center justify-center p-2 rounded">Remove from cart</button>
                    </li>
                )) : "Add items to your cart!"}
            </ul>
            <p className="pb-4">Your total: <span className="font-semibold">£{cart.cartItems.reduce((acc, curr) => acc + curr.price * curr.quantity, 0).toFixed(2)}</span></p>
            <button className="mx-auto block bg-slate-800 text-slate-200 py-2 px-4 rounded mt-7">Proceed to checkout</button>
        </div>
    </div>
  )
}
