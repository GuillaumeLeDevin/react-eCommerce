            ...
            <ul className="grid min-[500px]:grid-cols-2 md:grid-cols-3 gap-4">
                {products.items && products.items.map(product => (
                    <li key={product.id} className="p-4 bg-slate-200 rounded">
                        ...
                        className={`${product.picked ? "bg-green-700" : "bg-slate-600"} w-full text-slate-100 px-2 inline-flex items-center justify-center rounded p-2 mr-2`}>{product.picked ? "Item picked ✅" : "Add to cart" }</button>
                    </li>
                ))}
            </ul>
        </div>
